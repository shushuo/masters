...
| D9 | **默认数据源（v3.1 细化）** | **混合供给架构**：逐标的数据**客户端直连**公开渠道（适配器随云目录热更新）；全市场横截面（热榜等）由云端每日一份**静态快照**经 CDN 分发；云端代理仅做降级兜底。零配置保 D0 体验，免费档运营成本 ≈ 零；自配连接器可覆盖 |
| D10 | **关注时点快照** | 自动记录（价格+日期+关注原因，成本极低），但假设收益（观察仓）**只在季度复盘中由教练带行为金融框架呈现**，不做日常展示（避免助长 FOMO，与反焦虑定位冲突） |
| D11 | **上游姿态** | **法定披露渠道优先，公开转载渠道兜底**：公告/财报/基金净值取自法定披露渠道（巨潮等）；日终行情取自公开转载渠道并**双源交叉验证**；不采购商业转供授权（订阅档规模化后再评估为升级路径）；不碰实时行情 |
| D12 | **品牌** | 中文品牌名 **《大师》**（= Masters 直译，中英一体）；slogan **「像大师一样思考」**（主语是用户——升级你的思考，不是替你决定）；tagline **「你的私人投研团队」**；**更换熊猫吉祥物**（新吉祥物与视觉体系另行设计）。「大师」语境风险由五条话术护栏对冲（§13） |
| D13 | **空关注兜底** | **双层默认心跳**（§3.5）：日层「大师一句」（格言+教练解读，仅应用内不推送）+ 周层「本周市场三件事」（**云端策展 + 人工审核**、全员同一份、回顾不展望不出现个股、**默认订阅一键关**）。两层的成功指标都是**诱发提问的转化率**，不是阅读量——兜底是对话诱饵，不是内容产品 |
...
### 3.4 空关注兜底：产品的默认心跳（D13）

**问题定义**：兜底不是「给没有关注的人看的内容」，而是**当个人化触达无话可说时，产品的
默认心跳**。三类对象：S0 真空用户（从未提问——桌面端几乎不可达，主要靠 D0 空状态挽救，
诚实接受局限）、S1 弱关注用户（问过一两个，**主要服务对象**）、S2 安静周用户（有关注但
本周无事件——兜底同时解决「哨兵没话说的那几周产品失声」）。

**定位陷阱与成功指标**：兜底简报做成内容产品 = 和免费财经号竞争 + 编辑责任 + 观点风险，
与工具定位相冲。规避方式是换成功指标——**兜底的目的不是被阅读，而是诱发提问**（对话
诱饵，不是内容目的地）；每条内容固定结构：`事实（数据+时间戳）→ 与你何干（教育/框架
视角，非行动建议）→「就此提问」CTA`。CTA 进入专家团对话 → 聊出新关注 → 飞轮回到正轨
——**兜底的终点是让用户不再需要兜底**。
...
| 层 | 内容 | 节律与渠道 | 风险 |
|---|---|---|---|
| **日层「大师一句」** | 经典投资大师格言 + 教练一句引向自身的解读（「你最近问的三只票在你的能力圈里吗？」）——直接兑现「像大师一样思考」（D12 的内容红利） | **不推送**：对话页空状态轮换 + 简报流填充；卡片包走云目录分发 | 零合规风险（引用经典非观点输出）、零数据依赖 |
| **周层「本周市场三件事」** | 指数/板块/宏观/制度事件的周度回顾 | 默认订阅、一键关（应用内 + 可选通知/邮件）——反焦虑品牌下推送频率已是最低档，默认开才有兜底作用 | 三条护栏：**回顾不展望**（不定未来方向）；**不出现个股**（法定披露事件除外，如「财报季本周开启」）；每件事配教育角度、永不配行动暗示 |

**生产方式（周层）**：**云端每周策展一次、全员同一份**——LLM 起草 → 人工审核 → 随
横截面快照（§5.1）发布。每周一份的人审成本 ≈ 零，换来**可审核的单一合规面**；客户端
各自生成则是每用户一个合规敞口 + 烧用户 token，否决。客户端角色 = 渲染 + 承接
「就此提问」的后续对话。
...
- **M1 标的库 `AssetsServer`**（v2 的 PortfolioServer 重构为生命周期模型）：
  ...
  明细永不出设备；云端上下文可选脱敏。
- **M2 市场数据 `MarketDataServer`**（**升为 MVP 核心**，D9/D11，详设见 §5.1）：日频行情 +
  基本面快照 + 估值分位 + 公告/财报日历 + 新闻/热度，统一 schema + 「数据截至」戳 +
  本地缓存 + 双源校验元数据。混合供给（客户端直连为主 + 云端横截面快照 + 云端兜底）；
  自配 4d 连接器（Tushare 等，用户自担 ToS）可覆盖或补充。明确不做盘中/实时。
...
### 5.1 数据供给架构（M2 详设，D9/D11 决议的展开）

**需求盘点**——MVP 全部数据需求为日频或更低频（慢投资定位的直接红利）：

| 数据 | 用途 | 频率 | 覆盖 |
|---|---|---|---|
| 日终行情（收盘/涨跌/成交） | 研究卡快照、异动提醒、周报 | 每日 | 关注标的（人均 5–20） |
| 历史价格/净值序列 | 表现区间、估值分位、（V1）波动回撤 | 首拉 + 每日追加 | 按需 |
| 基金档案（经理/规模/费率/风格） | 研究卡「值得注意」 | 季频 | 按需 |
| 财务数据（→PE/PB 分位） | 研究卡估值段 | 季频 | 按需 |
| 公告/财报原文 + 财报日历 | 近期事件、财报哨兵 | 每日扫描 | 关注标的 |
| 全市场横截面（涨跌/成交榜） | 热度榜、「本周三件事」 | 每日 | 全市场，**所有用户共享一份** |

**合规分级**（慢投资定位使 MVP 全部落在最轻两档；正式上线前仍需专业合规意见确认）：

1. **法定公开披露信息（最轻）**：公告、财报原文（巨潮资讯为法定披露渠道）、基金净值
   （法定每日披露）、基金定期报告——公开传播本就是披露制度的目的。
2. **延迟/日终行情（较轻）**：实时 Level-1 转发需交易所信息公司授权计费；**日终与延迟
   15 分钟以上数据长期为各门户免费公开转载地带**——我们只需日终。
3. **实时行情（最重）**：因「不做盘中」（D6）完全不碰。

**混合供给架构（D9）**——按数据性质分工：

```mermaid
flowchart LR
    subgraph Client["桌面端（local-first）"]
        MDS["MarketDataServer\n统一 schema + price_cache\n(来源/时间戳/校验状态)"]
        AD["数据适配器\n(版本化，随云目录热更新)"]
    end
    subgraph Public["公开渠道"]
        CN["法定披露：巨潮公告/\n基金净值披露 (第一优先)"]
        PT["公开转载：日终行情\n(双源交叉验证)"]
    end
    subgraph Cloud["masters-cloud"]
        Snap["每日批任务 → 全市场横截面\n静态 JSON 快照 (CDN, 成本≈0)"]
        FB["兜底缓存代理\n(直连失败时降级)"]
    end
    MDS --> AD --> CN
...
- **逐标的数据客户端直连**：每个用户以个人身份获取公开披露信息（单用户请求量极小——
  人均十几个标的、日频），合规姿态最轻、云端成本归零、local-first 最纯。
- **上游改版的解药是现成的**：数据适配器做成**版本化内容随云目录（catalog sync）分发**
  ——上游页面改版后修适配器、推目录，不需要发版。
- **全市场横截面走云端**：热榜/「本周三件事」是全市场算一次、人人相同的结果——云端
  每日批任务产出一份静态 JSON 由 CDN 分发，边际成本约等于零。
- **云端代理只做兜底**：直连失败时降级，cloud 不成为运行时硬依赖（local-first 叙事保住）。
- **成本结构结论**：免费档运营成本 ≈ 一个每日批任务 + CDN 静态文件——**免费档不是烧钱
  承诺**；订阅档数据价值主张 = 云端兜底可用性 + 更全字段 + 更深历史，而非基础数据本身。

**「永不给错数」在数据层的落地——双源交叉验证**：关键数字（收盘价/净值）从两个独立
来源取，一致才入缓存标绿；不一致标「数据存疑」并显式降级（研究卡该字段显示 ⚠ 而非
任选其一）。来源与校验状态记入 `price_cache` 元数据，「计算依据」抽屉可见——信任前置
原则延伸到数据层。

**候选上游速览**：巨潮（公告/财报，免费，风险≈0）、基金披露渠道（净值/费率/定期报告，
免费，适配器跟进页面变更）、公开转载行情源（免费，双源验证对冲单源错数）、AKShare
（开发期参考实现；Python 依赖不随桌面端分发）、Tushare Pro（**ToS 禁转供**——仅作用户
自配连接器）、商业数据商（聚源/通联/米筐等，数万~数十万/年——订阅档规模化后的升级
路径，MVP 不采购）。
...
数据成本结构已由 §5.1 的混合供给架构解决：免费档运营成本 ≈ 每日批任务 + CDN 静态
文件；商业数据商采购是订阅档规模化后的升级路径，不压在免费档上。
...
| FR-INV-4b | **空关注兜底双层心跳**（§3.4）：日层「大师一句」（空状态轮换，卡片包云目录分发）+ 周层「本周市场三件事」（云端策展+人审，默认订阅一键关，逐条「就此提问」CTA） | **P0 (MVP)** |
...
| 层 | 内容 |
|---|---|
| 中文品牌名 | **《大师》** —— Masters 的直译；品牌/引擎名/域名（getmasters.app）完全统一，无需维护两套品牌资产 |
| slogan | **「像大师一样思考」** —— 关键的语义翻转：**主语是用户**。不是「跟着大师操作」（荐股话术的语法），而是你像巴菲特/芒格/格雷厄姆意义上的大师那样思考。产品承诺 = 升级你的思考，与「给你依据，不给答案」的合规站位自洽 |
| tagline | **「你的私人投研团队」** —— 一句话讲清产品形态（专家团），兼带私密/本地暗示 |
| 支撑句（官网/商店详情固定三条） | 「数字不出错」（确定性计算）·「来源可查」（引用链）·「持仓不出门」（本地隐私） |
| 应用商店命名 | 「大师 · 你的私人投研团队」（品牌名 + 品类副标，兼顾记忆与搜索） |

### 已知风险与五条话术护栏

「大师/老师」在中文投资语境曾被杀猪盘话术污染（「跟着大师操作」「群里老师带单」）。
命名决议已定，区隔靠**话术纪律**执行：

1. **产品内专家永远不称「大师」**——他们叫研究员/风控官/教练；**大师是用户要成为的人，
   不是我们扮演的角色**（这是整个品牌的语义支点）。
2. 文案永远不出现「跟着/听/带」+ 大师的动宾结构；「像大师一样思考」是唯一标准句式。
3. 大师意象锚定**经典投资大师的智慧**（引用、格言、原著——与 M10 学习模块、教练的
   行为金融视角天然联动），与「群里老师」的气质彻底切割。
4. 视觉走**克制的新中式极简**，不走财富金色；永不用感叹号谈收益，数据永远带时间戳
   ——文案规范是合规体系的一部分。
5. 免责体系照旧前置（§7 三层话术不变）。

### 视觉方向（吉祥物更换，D12）

熊猫吉祥物退役（现有 logo 与奶油色体系由它派生，docs/10）。新方向待专项设计，约束与
候选：吉祥物不得含「熊」字/意象；气质须贴「大师 = 智慧、守望、沉静」——候选如
**猫头鹰**（智慧 + 守望，「替你盯着」的形象化）、**仙鹤**（东方大师气质、超然不逐流）、
或**去吉祥物的字标 + 印章/水墨意象**（最克制）。配色体系随吉祥物定稿后重校
（docs/10 的 token 架构使换色是一次变量替换，不是推倒重来）。

---

## 14. 待决问题（下一轮）

1. **数据供给的收尾作业**（架构已定 §5.1，余两项）：①正式合规意见确认「日终行情公开
   转载」与「披露信息聚合」的边界；②适配器工程选型——内置 Rust 适配器（getmasters-mcp
   内实现少量端点，AKShare 作开发期参考）vs 随目录分发的独立二进制连接器。
